...
// Source: Networking exercise in CSC213
...
static int server_socket_accept(int server_socket_fd) {
  ...
  struct timeval timeout;      
    ...
    if (setsockopt (server_socket_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                sizeof timeout) < 0) {
        return -1;
        }

    if (setsockopt (server_socket_fd, SOL_SOCKET, SO_SNDTIMEO, &timeout,
                sizeof timeout) < 0) {
        return -1;
        }
